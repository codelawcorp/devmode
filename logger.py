...
class CustomFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        super().format(record)
        timestamp = datetime.datetime.utcfromtimestamp(record.created).strftime('%Y-%m-%d %H:%M:%S')
        ...
        if record.levelno == logging.DEBUG:
            ...
            log_record = f"{color}{timestamp} {record.levelname} {record.name} {context} {message}{COLORS['RESET']}"
        else:
            log_record = f"{color}{timestamp} {record.levelname} {record.name} {message}{COLORS['RESET']}"

...
def find_workspace_secret(workspace_name, namespace):
    ...
    try:
        secrets = V1_API.list_namespaced_secret(
            namespace=namespace,
        ...
        for secret in secrets.items:
            if secret.data.get("workspace_name"):
                decoded_name = base64.b64decode(secret.data["workspace_name"]).decode('utf-8')
                if decoded_name == workspace_name:
                    return secret
                    
        logger.error(f"No workspace found with name {workspace_name} in namespace {namespace}")
        sys.exit(1)
...
def start_workspace(deployment_name, namespace="default", workspace_name = None, workspace_path = "/app"):
    ...
    try:
        ...
        secret_result = V1_API.create_namespaced_secret(
            namespace=namespace,
            body=secret
        )
    ...
    return WORKSPACE_NAME, deployment_result.metadata.name, pvc_result.metadata.name, secret_result.metadata.name
...
def delete_workspace(workspace_name, namespace="default"):
    ...
    secret = find_workspace_secret(workspace_name, namespace)
    secret_name = secret.metadata.name
    try:
        ...
        if deployment_name:
            logger.debug(f"Attempting to delete deployment {deployment_name}")
            # Delete deployment
            try:
        ...
        if pvc_name:
            logger.debug(f"Attempting to delete PVC {pvc_name}")
            # Delete PVC
            try:
                V1_API.delete_namespaced_persistent_volume_claim(
                    name=pvc_name,
...
def recreate_workspace(workspace_name, namespace="default", new_workspace_path=None):
    ...
    # Start new workspace
    start_workspace(original_deployment_name, namespace, workspace_name=workspace_name, workspace_path=new_workspace_path or workspace_path)
